...
std::weak_ordering GroceryItem::operator<=>( const GroceryItem & rhs ) const noexcept
{
  ...
  ///////////////////////// TO-DO (19) //////////////////////////////
  if (auto upc = _upcCode <=> rhs._upcCode; upc != 0) {
    return upc;
  } 
  if (auto product = _productName <=> rhs._productName; product != 0) {
    return product;
  } 
  if (auto brand = _brandName <=> rhs._brandName; brand != 0) {
    return brand;
  } 
...
std::istream & operator>>( std::istream & stream, GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (21) //////////////////////////////
    ...
    if (stream >> std::quoted(upcCode) &&
        ...
        stream >> std::quoted(brandName) &&
        stream.ignore(std::numeric_limits<std::streamsize>::max(), ',') &&
        stream >> std::quoted(productName) &&
        stream.ignore(std::numeric_limits<std::streamsize>::max(), ',') &&
        stream >> price) 
    {
      GroceryItem tempItem(productName, brandName, upcCode, price);
      groceryItem = std::move(tempItem);
    }
    return stream;
  /////////////////////// END-TO-DO (21) ////////////////////////////
}
