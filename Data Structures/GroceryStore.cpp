...
GroceryStore::GroceryStore( const std::string & persistentInventoryDB )
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    std::string upc;
    std::size_t quantity;
    while (fin >> std::quoted(upc) && fin >> quantity) {
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomers( const ShoppingCarts & shoppingCarts, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (3) //////////////////////////////
    ...
    for (const auto& shoppingCart : shoppingCarts) {
      receipt << '\n' << shoppingCart.first << "'s shopping cart contains:\n";
...
GroceryStore::GroceryItemsSold GroceryStore::ringUpCustomer( const ShoppingCart & shoppingCart, std::ostream & receipt )
{
  ...
  ///////////////////////// TO-DO (4) //////////////////////////////
    ...
    for (const auto& item : shoppingCart) {
      const auto current = worldWideGroceryDatabase.find(item.first);
      if (current == nullptr) {
        receipt << std::quoted(item.first) 
                  << " (" << item.second.productName() << ") "
                  << "not found - You get it free! Hooray!\n";
      } else {
        receipt << *current << '\n';
        amountDue += current->price();
        auto theItem = _inventoryDB.find(item.first);
        if (theItem != _inventoryDB.end() && theItem->second > 0) {
          --theItem->second;
          purchasedGroceries.insert(theItem->first);
        }
      }
    }
    receipt << "-------------------------\nTotal  $" << amountDue << '\n';
...
void GroceryStore::reorderItems( GroceryItemsSold & todaysSales, std::ostream & reorderReport )
{
  ...
  ///////////////////////// TO-DO (5) //////////////////////////////
    ...
    std::size_t numItemsReordered = 1;
    for (const auto& item : todaysSales) {
      const auto exists = worldWideGroceryDatabase.find(item);
      const auto inStore = _inventoryDB.find(item);
      if (inStore == _inventoryDB.end() || inStore->second < REORDER_THRESHOLD) {
        reorderReport << "\n " << numItemsReordered << ":  {";
        if (exists == nullptr) reorderReport << item;
        else reorderReport << *exists;
        reorderReport << "}\n";
        if (inStore == _inventoryDB.end()) reorderReport << exists->productName() 
                                                         << " is no longer sold in this store and will not be re-ordered\n";
        else {
          reorderReport << "        only " << inStore->second << " remain in stock which is "
                        << REORDER_THRESHOLD - inStore->second << " unit(s) below reorder threshold (15), re-ordering "
                        << LOT_COUNT << " more\n";
          inStore->second += LOT_COUNT;
        }
        ++numItemsReordered;
      }
    }
