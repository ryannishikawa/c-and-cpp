...
int main( int argc, char * argv[] )
{
  try
  {
    ...
    ///////////////////////// TO-DO (7) //////////////////////////////
      ...
      /// no charge.
      GroceryItem * currentRN = nullptr;
      while (!checkoutCounter.empty()) {
        currentRN = worldWideDatabase.find(checkoutCounter.front().upcCode());
        //std::cout << "\n~~~~~~~~~~~~~~~~~~~~\ncurrentRN = " << currentRN << "\n~~~~~~~~~~~~~~~~~~~~\n";                              }
        //std::cout << "\n~~~~~~~~~~~~~~~~~~~~\ncurrent upc = " << checkoutCounter.front().upcCode() << "\n~~~~~~~~~~~~~~~~~~~~\n";    } for debugging logic errors
        if (currentRN == nullptr) {
          std::cout << std::quoted(checkoutCounter.front().upcCode()) 
                    << " (" << checkoutCounter.front().productName() << ") "
                    << "not found, so today is your lucky day - You get it free! Hooray!\n";
        } else {
          amountDue += currentRN->price();
          std::cout << *currentRN << '\n';
        }
        checkoutCounter.pop();
      }
