...
struct BST
{
  ...
  *****************************************************************************/
  ...
  Node * find( Key const & key, Node * current )
  {
    ...
    else if(comp < 0) return find(key, current->_left);
  ...
  *****************************************************************************/
  void erase( Key const & key )
  {
    ...
    if(pos->_right && pos->_left) {
      Node * successor = pos->_right;
      while (successor->_left != nullptr) successor = successor->_left;
      const_cast<Key &>(pos->_pair.first) = const_cast<Key &&>(successor->_pair.first);
                        pos->_pair.second = std::move(successor->_pair.second);
      erase(successor->_pair.first);
    } else {
      Node ** parent = pos->_parent == nullptr ? & _root
                      :pos->_parent->_left == pos ? & pos->_parent->_left
                      :                             & pos->_parent->_right;
      Node * child = pos->_left ? pos->_left : pos->_right;
      *parent = child;
      if(child) child->_parent = pos->_parent;
      delete pos;
      --_size;
    }
    /////////////////////// END-TO-DO (4) ////////////////////////////
