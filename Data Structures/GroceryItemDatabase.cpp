...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    GroceryItem item = {};
    while (fin >> item) {
      _dataStoreRN.push_back((std::move(item)));
    }
  /////////////////////// END-TO-DO (2) ////////////////////////////
...
///////////////////////// TO-DO (3) //////////////////////////////
  ...
  GroceryItem * GroceryItemDatabase::find(const std::string & upcRN) {
    return find(_dataStoreRN.begin(), _dataStoreRN.begin() + _dataStoreRN.size(), upcRN);
  }
  GroceryItem * GroceryItemDatabase::find(std::vector<GroceryItem>::iterator currentRN, std::vector<GroceryItem>::iterator endRN, const std::string & upcRN) {
    if (endRN == currentRN) return nullptr;
    if ((currentRN)->upcCode() == upcRN) return (&_dataStoreRN[std::distance(_dataStoreRN.begin(), currentRN)]);
    return find(++currentRN, endRN, upcRN);
  }

  std::size_t GroceryItemDatabase::size() const {
    return _dataStoreRN.size();
  }
/////////////////////// END-TO-DO (3) ////////////////////////////
