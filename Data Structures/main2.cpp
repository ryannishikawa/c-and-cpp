...
int main()
{
  try
  {
    ...
    ///////////////////////// TO-DO (1) //////////////////////////////
     ...
     //wanna make fried chicken and earbuds stopped working recently
     GroceryList stuffINeed = {{"chicken thigh"},
                               {"cornstarch"}, 
                               {"earbuds", "Apple"}};
    //checking what's on the list
     std::cout << stuffINeed << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~";
     //kinda want soy garlic
     stuffINeed.insert({"honey"}, stuffINeed.find({"cornstarch"}));
     stuffINeed.insert({"soy sauce", "Kikkoman"});
     stuffINeed.insert({"garlic"}, stuffINeed.size());
     stuffINeed.insert({"butter"});
     //friend also wants some stuff
     GroceryList friendsList;
     friendsList += {{"cheetos", "Cheetos"},
                     {"soda", "Sprite"}};
     //im already going so may as well get his stuff too
     if (stuffINeed != friendsList) {
      //double checking the list
      if (auto tempList =  stuffINeed <=> friendsList; tempList != 0)
      stuffINeed += friendsList;
     }
     //already have butter at home
     stuffINeed.remove({"butter"});
     //check the list again
     std::cout << stuffINeed << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~";
     //should prob get the earbuds first
     stuffINeed.moveToTop({"earbuds", "Apple"});
     //friend is broke so taking their stuff off
     stuffINeed.remove({"cheetos", "Cheetos"});
     stuffINeed.remove({"soda", "Sprite"});
     //final check of list
     std::cout << stuffINeed << "\n~~~~~~~~~~~~~~~~~~~~~~~~~~~";
     //checking list 
     std::istringstream expectedListStream(R"( "",   "Apple",            "earbuds",          0.0
                                         "",   "Kikkoman",            "soy sauce",          0.0
                                         "",   "",            "chicken thigh",      0.0
                                         "",   "",  "honey",        0.0
                                         "",   "",            "cornstarch",         0.0
                                         "", "", "garlic", 0.0)");
     GroceryList expectedList;
     expectedListStream >> expectedList;
     std::cout << "\nExpected results:" << expectedList << "\n\n"
               << "\nActual results:  " << stuffINeed     << "\n\n"
               << "\nTest results:    " << ( stuffINeed == expectedList ? "PASS" : "FAIL" ) << '\n';
    /////////////////////// END-TO-DO (1) ////////////////////////////
  }
