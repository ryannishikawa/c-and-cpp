...
struct SLL
{
  ...
  Node * insert_after( Node * position, T const & data )
  {
    ...
    Node *rnNewNode = new Node(data);
    Node *rnCurrentNode = position;

    if (_size == 0) {
      rnHead = rnTail = rnNewNode;
    } else if (position == nullptr) {
      rnNewNode->_next = rnHead;
      rnHead = rnNewNode;
    } else if (rnCurrentNode == rnTail) {
      rnTail->_next = rnNewNode;
      rnTail = rnNewNode;
    } else {
      rnNewNode->_next = rnCurrentNode->_next;
      rnCurrentNode->_next = rnNewNode;
    }
    ++rnSize;
    return rnNewNode;
    /////////////////////// END-TO-DO (1) ////////////////////////////
