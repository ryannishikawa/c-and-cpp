...
// everywhere it is used.  Note:  C++20 will change this technique with the introduction of the std::source_location class. Also note the
// usage of having the preprocessor concatenate two string literals separated only by whitespace.  :(  In the meantime ...
//
// Update:  As of August 2023 and clang 16, still no support for source_location. Rummer has it that they may never implement it.
// Yay Microsoft and GCC!
  ...
  { /**********  Part 1 - Insert into array  ***********************/
    ///////////////////////// TO-DO (4) //////////////////////////////
      ...
      if (_gList_array_size < _gList_array.size()) {
        std::move_backward(_gList_array.begin() + offsetFromTop, _gList_array.begin() + _gList_array_size, _gList_array.begin() + _gList_array_size + 1);
        _gList_array[offsetFromTop] = groceryItem;
        ++_gList_array_size;
      } else {
        throw CapacityExceeded_Ex ("List is full" exception_location);
      }
    /////////////////////// END-TO-DO (4) ////////////////////////////
  } // Part 1 - Insert into array
...
void GroceryList::remove( std::size_t offsetFromTop )
{
  ...
  { /**********  Part 1 - Remove from array  ***********************/
    ///////////////////////// TO-DO (8) //////////////////////////////
      ...
      /// in your array, so don't forget to adjust _gList_array_size.
      std::move(_gList_array.begin() + offsetFromTop + 1, _gList_array.begin() + _gList_array_size, _gList_array.begin() + offsetFromTop);
      --_gList_array_size;
...
void GroceryList::moveToTop( const GroceryItem & groceryItem )
{
  ///////////////////////// TO-DO (12) //////////////////////////////
    ...
    const std::size_t posRN = find(groceryItem);
    if (posRN != _gList_array_size) {
      remove(posRN);
      insert(groceryItem, GroceryList::Position::TOP);
    } else {
      return;
    }
  /////////////////////// END-TO-DO (12) ////////////////////////////
...
GroceryList & GroceryList::operator+=( const std::initializer_list<GroceryItem> & rhs )
{
  ///////////////////////// TO-DO (13) //////////////////////////////
    ...
    for (const auto& itemRN : rhs) {
      insert(itemRN, GroceryList::Position::BOTTOM);
    }
  /////////////////////// END-TO-DO (13) ////////////////////////////
...
GroceryList & GroceryList::operator+=( const GroceryList & rhs )
{
  ///////////////////////// TO-DO (14) //////////////////////////////
    ...
    for (std::size_t i = 0; i < rhs.size(); ++i) {
      insert(rhs._gList_array[i], _gList_array_size);
    }
  /////////////////////// END-TO-DO (14) ////////////////////////////
...
std::weak_ordering GroceryList::operator<=>( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (15) //////////////////////////////
    ...
    std::size_t smolListSizeRN = 0;
    if (_gList_array_size >= rhs._gList_array_size) {
      smolListSizeRN = _gList_array_size;
    } else {
      smolListSizeRN = rhs._gList_array_size;
    }

    for (std::size_t i = 0; i < smolListSizeRN; ++i) {
      if (auto tempItemRN = _gList_array[i] <=> rhs._gList_array[i]; tempItemRN != 0) {
        return tempItemRN;
      }
    }
...
bool GroceryList::operator==( GroceryList const & rhs ) const
{
  ...
  ///////////////////////// TO-DO (16) //////////////////////////////
    ...
    if (_gList_array_size == rhs._gList_array_size) {
      for (std::size_t i = 0; i < _gList_array_size; ++i) {
        if (_gList_array[i] != rhs._gList_array[i]) {
          return false;
        }
      }
      return true;
...
std::istream & operator>>( std::istream & stream, GroceryList & groceryList )
{
  ...
  ///////////////////////// TO-DO (18) //////////////////////////////
    ...
    auto tempItemRN = GroceryItem();
    while (stream >> tempItemRN) {
      groceryList.insert((tempItemRN), groceryList.size());
    }
  /////////////////////// END-TO-DO (18) ////////////////////////////
